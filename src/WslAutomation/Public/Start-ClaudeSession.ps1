function Start-ClaudeSession {
    ...
    .DESCRIPTION
        ...
        interactive login shell running 'claude --remote-control'. Intended to be called only
        after Test-ClaudeSession has confirmed no session is already running, so callers do not
        end up with duplicate sessions.
    ...
    .PARAMETER ArgumentList
        ...
        'wsl.exe -d <DistroName> --cd ~ -- bash -l -c "claude --remote-control"'. See
        Get-ClaudeSessionWtArgumentList for why the title and the claude command are pre-quoted,
        and why the profile is selected.
    .EXAMPLE
        Start-ClaudeSession -DistroName 'Ubuntu'

        Opens a new Windows Terminal tab running Claude Code with Remote Control enabled inside
        the 'Ubuntu' distro.
    #>
